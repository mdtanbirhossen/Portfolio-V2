...
export default function Title({ title, subtitle, className }: { title: string, subtitle?: string, className?: string }) {
    return (
        <div className={`flex flex-col justify-center text-center w-full space-y-2 md:space-y-4 mb-10 ${className}`}>
            <h3 data-animation="text" className='text-primary-dark dark:text-primary-light text-xl md:text-2xl lg:text-4xl xl:text-5xl'>{title}</h3>
